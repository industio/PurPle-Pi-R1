...
HalDispInternalConfig_t gstDispInterCfg = {E_HAL_DISP_INTER_CFG_NONE, 0, HAL_DISP_COLOR_CSC_ID};
...
void __HalDispIfSetCscConfig(HalDispCsc_t *pstCscCfg)
{
    ...
    {
        HalDispColorSeletYuvToRgbMatrix(gstDispInterCfg.u8ColorId, E_DISP_COLOR_YUV_2_RGB_MATRIX_BYPASS, NULL);
    }
    else if(pstCscCfg->eCscMatrix == E_HAL_DISP_CSC_MATRIX_BT601_TO_RGB_PC)
    {
        HalDispColorSeletYuvToRgbMatrix(gstDispInterCfg.u8ColorId, E_DISP_COLOR_YUV_2_RGB_MATRIX_SDTV, NULL);
    }
    else if(pstCscCfg->eCscMatrix == E_HAL_DISP_CSC_MATRIX_BT709_TO_RGB_PC)
    {
        HalDispColorSeletYuvToRgbMatrix(gstDispInterCfg.u8ColorId, E_DISP_COLOR_YUV_2_RGB_MATRIX_HDTV, NULL);
    }

    HalDispColorAdjustHCS(gstDispInterCfg.u8ColorId, pstCscCfg->u32Hue, pstCscCfg->u32Saturation, pstCscCfg->u32Contrast);
    HalDispColorAdjustBrightness(gstDispInterCfg.u8ColorId, pstCscCfg->u32Luma, pstCscCfg->u32Luma, pstCscCfg->u32Luma);
}
...
void _HalDispIfSetDeviceInit(void *pCtx, void *pCfg)
{
    ...
    if(stPqColorId.eType == E_HAL_DISP_INTER_CFG_COLORID)
    {
        gstDispInterCfg.eType = stPqColorId.eType;
        gstDispInterCfg.bBootlogoEn = stPqColorId.bBootlogoEn;
        gstDispInterCfg.u8ColorId = stPqColorId.u8ColorId;
    }
    ...
    if(PQ_PICTURE_EN)
    {
        HalDispColorAdjustVideoRGB(gstDispInterCfg.u8ColorId,
            PQ_PICTURE_R_GAIN,
            PQ_PICTURE_G_GAIN,
            PQ_PICTURE_B_GAIN);

        HalDispColorAdjustHCS(gstDispInterCfg.u8ColorId,
            PQ_PICUTRE_HUE,
            PQ_PICTURE_SATURATION,
            PQ_PICTURE_CONTRAST);

        HalDispColorAdjustBrightness(gstDispInterCfg.u8ColorId,
            PQ_PICTURE_R_BRIGHTNESS,
            PQ_PICTURE_G_BRIGHTNESS,
            PQ_PICTURE_B_BRIGHTNESS);
    }
    else
    {
        HalDispColorAdjustHCS(gstDispInterCfg.u8ColorId,
        ...
        HalDispColorAdjustBrightness(gstDispInterCfg.u8ColorId,
            pstDeviceContain->stHdmiParam.stCsc.u32Luma,
            pstDeviceContain->stHdmiParam.stCsc.u32Luma,
            pstDeviceContain->stHdmiParam.stCsc.u32Luma);
    }
    ...
    if(PQ_PEAKING_EN)
    {
        HalDispPqSetPeaking(&stPqPeaking);
    }

    if(PQ_GAMMA_EN)
    {
        HalDispPqSetGammaConfig(&stPqGamma);
    }
}
...
void _HalDispIfSetDeviceColorTemp(void *pCtx, void *pCfg)
{
    ...
    pstColorTemp = (HalDispColorTemp_t *)pCfg;
    HalDispColorAdjustVideoRGB(gstDispInterCfg.u8ColorId, pstColorTemp->u16RedColor, pstColorTemp->u16GreenColor, pstColorTemp->u16BlueColor);
}
