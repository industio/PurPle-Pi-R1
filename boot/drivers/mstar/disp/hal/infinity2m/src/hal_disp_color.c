...
void SetBrightness(u8 u8Id, u8 u8BrightnessR, u8 u8BrightnessG, u8 u8BrightnessB)
{
    if(u8Id == 0)
    {
        W2BYTEMSK(REG_DISP_TOP_OP2_3A_L, ((u16)u8BrightnessR) << 0, 0x00FF);
        W2BYTEMSK(REG_DISP_TOP_OP2_3A_L, ((u16)u8BrightnessG) << 8, 0xFF00);
        W2BYTEMSK(REG_DISP_TOP_OP2_3B_L, ((u16)u8BrightnessB) << 0, 0x00FF);
    }
    else
    {
        W2BYTEMSK(REG_DISP_TOP_OP2_46_L, ((u16)u8BrightnessR) << 0, 0x00FF);
        W2BYTEMSK(REG_DISP_TOP_OP2_46_L, ((u16)u8BrightnessG) << 8, 0xFF00);
        W2BYTEMSK(REG_DISP_TOP_OP2_47_L, ((u16)u8BrightnessB) << 0, 0x00FF);
    }
}
...
void HalDispColorInitVar(void)
{
    ...
    for(i=0; i<2; i++)
    {
        ...
        g_tColorCfg[i].u8BrightnessR = 0x80;
        g_tColorCfg[i].u8BrightnessG = 0x80;
        g_tColorCfg[i].u8BrightnessB = 0x80;
...
void HalDispColorAdjustBrightness( u8 u8Id, u8 u8BrightnessR, u8 u8BrightnessG, u8 u8BrightnessB)
{
    DISP_DBG(DISP_DBG_LEVEL_COLOR, "%s %d Id:%d, Brightness=%d %d %d\n",
        __FUNCTION__, __LINE__, u8Id, u8BrightnessR, u8BrightnessG, u8BrightnessB);

    g_tColorCfg[u8Id].u8BrightnessR = u8BrightnessR;
    g_tColorCfg[u8Id].u8BrightnessG = u8BrightnessG;
    g_tColorCfg[u8Id].u8BrightnessB = u8BrightnessB;

    SetBrightnessEn(u8Id, 1);
    SetBrightness(u8Id, u8BrightnessR, u8BrightnessG, u8BrightnessB);
}
