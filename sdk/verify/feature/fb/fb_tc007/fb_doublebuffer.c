...
int main(int argc, char **argv)
{
    ...
    RECT rect = {100, 100, 10, 300};
    RECT rectb = rect;
    RECT rectb1 = rect;
    int fb = 0;
    int ret = 0;
#define VSYNC_UNSUPPORTED
    ...
    while(1)
    {
        if(fb)
        {
            ...
            drawRect(rectb.x, rectb.y, rectb.w, rectb.h, 0xffffffff);
            drawRect(rect.x, rect.y, rect.w, rect.h, 0xffff0000);

            vinfo.yoffset = 0;
            //Pandisplay
#ifdef VSYNC_UNSUPPORTED
            ...
            if((ret = ioctl(fbFd, FBIO_WAITFORVSYNC)) < 0)
            {
                printf("Error: failed to FBIO_WAITFORVSYNC %d \n", ret);
                break;
            }
        ...
        {
            frameBuffer0 = frameBuffer + vinfo.yres * finfo.line_length;

            drawRect(rectb1.x, rectb1.y, rectb1.w, rectb1.h, 0xffffffff);
            drawRect(rect.x, rect.y, rect.w, rect.h, 0xffff0000);
            fb = 1;
            rectb1.x = rect.x;
            //Pandisplay
