...
static int ms_uart_rs485_gpio(struct serial_rs485 *rs485,bool send)
{
    ...
    if ((rs485->flags & SER_RS485_ENABLED) && send)
    {
        ...
        {
            gpio_set_value(rs485->padding[0],1);UART_DBG("wend set higt\n");
        }
        else
        {
            gpio_set_value(rs485->padding[0],0);UART_DBG("send set low\n");
        }
    }
    else if ((rs485->flags & SER_RS485_ENABLED) && !send)
    {
        ...
        {
            gpio_set_value(rs485->padding[0],1);UART_DBG("after set higt\n");
        }
        else
        {
            gpio_set_value(rs485->padding[0],0);UART_DBG("after set low\n");
        }
    }
    return 0;
}
